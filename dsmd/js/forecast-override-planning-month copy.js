  ...
  async function getData() {
    try {
      ...
      mergedData.forEach((item) => {
        if (item.CUSTOMER_NAME !== currentCustomer) {
          // New customer group encountered, sort and append the current group to sortedData
          ...
          // Same customer group, add item to the current group
          currentGroup.push(item);
        }
      });

      // Sort and append the last group
      if (currentGroup.length > 0) {
        sortGroupAndAppend();
      }
      ...
      function sortGroupAndAppend() {
        ...
        sortedData.data.push(...sortedGroup);
      }

      isTesting && handleResp(sortedData); // testing
    }
  }
