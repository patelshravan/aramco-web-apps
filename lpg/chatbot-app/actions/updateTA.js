export function updateTerminalAvailsAdjustment({ product, startDay, endDay, value }) {
    ...
    if (product.toLowerCase() === 'all') {
        ...
        if (!singleCode || typeof singleCode !== "string") {
            addChat("bot", `❌ Unknown or invalid product: "${product}"`);
            return false;
        }
        productCodes = [singleCode];
    }

    const adjVal = parseFloat(value);
    ...
    if (endDate < now) {
        addChat("bot", `⚠️ Both start day (${start}) and end day (${end}) are in the past. Please select today or future days.`);
        return false;
    }
        ...
        productCodes.forEach(productCode => {
            if (isInternational) {
                ...
                locations.forEach(location => {
                    const field = `adjustment_${productCode}_${location}TA`;
                    const rowKey = grid.getKeyByRowIndex(rowIndex);
                    grid.cellValue(rowKey, field, adjVal);
                    data[rowIndex][field] = adjVal;
                    updatedCount++;
                });
            } else {
                const field = `adjustment_${productCode}TA`;
                const rowKey = grid.getKeyByRowIndex(rowIndex);
                grid.cellValue(rowKey, field, adjVal);
                data[rowIndex][field] = adjVal;
                updatedCount++;
