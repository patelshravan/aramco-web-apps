...
$(document).ready(function () {
  ...
  // Fetch Latest Data
  $("#fetchLatestDataBtn").on("click", async function (e) {
    e.preventDefault();
    await callReadAPIWithSelection(window.selection, true);
  });
  ...
  $("#viewInScenarioSpotBtn").off("click").on("click", function () {
    ...
    const spotNominations = (window.spotNominations || []).filter(
      suggestion => selectedDates.includes(suggestion.DATE_VALUE.toString())
    );
  ...
  function revertNominationDetailForDate(date, suggestionsForDate) {
    ...
    suggestionsForDate.forEach(suggestion => {
      const { NOMINATION_NO } = suggestion;
      ...
      if (nomination.isSpotInjected) {
        parentRow.nomination.splice(nomIndex, 1);
        revertedAny = true;
      } else {
        selection.products.forEach(product => {
          const adjKey = `adjustedQty_${product}`;
      ...
      // Recalculate adjustment_CL
      selection.products.forEach(product => {
        parentRow[`adjustment_${product}CL`] = parentRow.nomination
          .reduce((sum, n) => sum + (n[`adjustedQty_${product}`] || 0), 0);
      });
    });

    if (revertedAny) {
      if (parentRow.nomination.length === 0) {
        selection.products.forEach(productCode => {
          parentRow[`adjustment_${productCode}CL`] = 0;
          parentRow[`adjustment_${productCode}TA`] = 0;
        });
      }
      ...
      const mainGrid = $("#liftingAmendmentGrid").dxDataGrid("instance");
      mainGrid.option('dataSource', liftingAmendmentData);
      mainGrid.refresh();
  ...
  async function handleReadApiData(data) {
    try {
      ...
      data.UOM?.forEach((item) => {
        ...
        const selectedUnit = uniqueUnits.includes("KMT") ? "KMT" : uniqueUnits[0];
        $unitSelect.val(selectedUnit);
  ...
  async function callReadAPIWithSelection(selection, useFetchLatest = false) {
    try {
      $("#loadingSpinner").show();
      ...
      // const apiEndpoint = useFetchLatest
      //   ? API_ENDPOINTS.INVPROJ_FETCH_LATEST_DATA
      //   : API_ENDPOINTS.READ;
  ...
  function createGrid(
    ...
    function generateColumns() {
      ...
      // --- PATCH: Opening Inventory & Working Capacity for INTERNATIONAL ---
      // Detect if this is Opening Inventory or Working Capacity modal
  ...
  `);
    ...
    productPairs.forEach(({ product, location }) => {
      ...
      console.log(`${product} - demand: ${demand}, factor: ${factor}, converted: ${demand * factor}`);
  ...
  function initializeLiftingAmendmentGrid() {
    ...
    // Only now, after countryMap is ready, call recalculate
    recalculateLiftingData();

    // Build unique product + location pairs
    let productPairs = [];
    (inventoryData || []).forEach(entry => {
      if (selection.products.includes(entry.PRODUCT_CODE)) {
        const key = `${entry.PRODUCT_CODE}_${entry.LOCATION}`;
    ...
    if (isInternational) {
      ...
      Object.entries(groupedByLocation).forEach(([location, products]) => {
        productColumns.push({
          caption: location,
          alignment: "center",
          columns: products.map(product => ({
            caption: window.productNameMap[product] || product,
            alignment: "center",
            columns: getProductFieldColumns(product, location)
          }))
        });
      });
    } else {
      // DOMESTIC → group only by PRODUCT
      selection.products.forEach((product) => {
        productColumns.push({
          caption: window.productNameMap[product] || product,
          alignment: "center",
      ...
      masterDetail: {
        ...
        template: function (container, options) {
          ...
          const scheduledColumns = selection.products.map((product) => ({
            ...
            caption: (window.productNameMap && window.productNameMap[product]) || product,
            width: 120, // Increased width for full product name
            alignment: "center",
            allowEditing: false,
            allowSorting: false,
            format: "#,##0",
          ...
          // only add if international
          if (isInternational) {
            nominationColumns.push(
              {
                dataField: "location",
                caption: "Location",
                width: 100,
                ...
                { caption: "Adjusted Qty", columns: adjustedColumns },
                {
                  ...
                  editorOptions: {
                    ...
                    onOpened: function (e) {
                      ...
                      if (calendar) {
                        calendar.option("currentDate", new Date(parseInt(yearStr), monthMap[monthStr], 1));
                      }
                    },
    ...
    function getProductFieldColumns(product, location = "") {
      const suffix = location ? `${product}_${location}` : product;
      ...
      return [
        ...
        },
        {
          ...
          headerCellTemplate: function (container, options) {
            // Use the product and location variables from the closure
            $(container).html(`
              <div style="display:flex; align-items:center; justify-content:center;">
                <span>Adjust</span>
                <button class="btn btn-link p-0 m-0 bulk-adjust-btn" data-type="TA" data-product="${product}" data-location="${location}" title="Bulk Adjust" style="font-size:14px;line-height:1;vertical-align:middle;display:inline-block;margin-left:4px;">
                  <i class="fa fa-edit"></i>
                </button>
              </div>
            `);
    ...
    // Make these accessible to chatbot.js
    ...
    function adjustmentTemplate(container, options) {
      ...
      const factor = unitConversionFactors[prod] || 1;
      const converted = val * factor;
      $(container).text(converted.toLocaleString('en-US', {
  ...
  $(document).on("click", "#confirmDeleteVersionsBtn", async function () {
    ...
    try {
      const deletePayload = {
        ...
        LOCATION: selection?.locationCode,
        PRODUCT_CODE: selection?.productCodes,
        VERSION_NO: selectedIds.map(id => Number(id)),
        PROCESS_ID: window.processId,
      };
      ...
      const csrfToken = await fetchCSRFToken();
      const response = await fetch(`${BASE_URL}${API_ENDPOINTS.INVPROJ_DELETE_VERSION}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRF-TOKEN": csrfToken,
